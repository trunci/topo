...
def build(s: dict, e: dict, x: dict, r: dict, g: dict, p: dict,
    ...
    # --- exp14 numbers (TOHA head-to-head: Mistral-7B on HotpotQA bridge) ---
    zqa = zq["all_items"]
    zq_n = zq["n_items"]
    zq_acc = _f(zq["overall_accuracy"], 3)
    zq_seqlen = _f(zq["mean_seq_len"], 0)
    zq_toha = _f(zq["toha_reference_auroc"], 2)
    zq_verdict = zqa["verdict"]
    zq_topo = _f(zqa["auc"]["topology_only"]["mean_auc"], 3)
    zq_conf = _f(zqa["auc"]["confidence_only"]["mean_auc"], 3)
    zq_ctrl = _f(zqa["auc"]["confidence_plus_controls"]["mean_auc"], 3)
    zq_full = _f(zqa["auc"]["confidence_plus_ctrl_topo"]["mean_auc"], 3)
    zq_d_conf_med = _f(zqa["delta_topo_vs_conf"]["median"], 3)
    zq_d_conf_p = _f(zqa["delta_topo_vs_conf"]["wilcoxon_p"], 4)
    zq_d_conf_adds = zqa["delta_topo_vs_conf"]["adds"]
    zq_d_ctrl_med = _f(zqa["delta_topo_vs_ctrl"]["median"], 3)
    zq_d_ctrl_p = _f(zqa["delta_topo_vs_ctrl"]["wilcoxon_p"], 4)
    zq_d_ctrl_adds = zqa["delta_topo_vs_ctrl"]["adds"]
...
head-to-head null against TOHA — that established all of this.
...
## 18. Result Q — TOHA head-to-head on real QA (Exp 14): {zq_verdict} (NULL)

Every failure-prediction result so far (Exp 7, 12, 13) used *synthetic* kinship/ordering
tasks. Exp 14 is the decisive external test: the **same model and benchmark as TOHA**
(Bazarova et al., ACL 2026) — Mistral-7B-Instruct-v0.3 on HotpotQA bridge questions
(distractor setting, full 10-paragraph contexts) — so our H1-persistence features compete
head-to-head against a published topological-attention method on naturalistic multi-hop QA.
n={zq_n} items, mean prompt length {zq_seqlen} tokens. HotpotQA validation bridge items are
uniformly labelled `hard`, so the whole sample sits at {zq_acc} accuracy — itself inside the
miscalibration band that made Exp 13 GREEN.

| feature set | failure-prediction AUC |
|---|---|
| confidence only | {zq_conf} |
| topology only | {zq_topo} |
| confidence + first-order controls | {zq_ctrl} |
| confidence + controls + topology | {zq_full} |
| TOHA (same model/data, paper-reported) | {zq_toha} |

- Topology-only AUC {zq_topo} — at chance, and **{zq_beats_toha} TOHA's {zq_toha}**.
- Topology adds beyond confidence: **{zq_d_conf_adds}** (Δ-AUC median {zq_d_conf_med},
  paired Wilcoxon p = {zq_d_conf_p}).
- Topology adds beyond first-order controls: **{zq_d_ctrl_adds}** (Δ-AUC median
  {zq_d_ctrl_med}, p = {zq_d_ctrl_p}).

**Verdict: {zq_verdict} — a clean NULL on real QA.** The H1 features are computed correctly
(real variance, no NaNs) and the base rate (acc {zq_acc}) sits exactly in the miscalibration
regime where synthetic deep-hop reasoning gave a strong GREEN (Exp 13, hop=5 topology AUC ≈
...
concept. Second, the regime-conditional claim (Result P) does **not** transfer from controlled
...
copy relation (F/E2). As a failure predictor it is **regime-conditional, but only on
synthetic tasks**: redundant with confidence on easy tasks (H, O — accuracy 77–85%), adding
distinctly when a small model is near its capability ceiling on clean, length-controlled
...
2026, arXiv 2504.10063) independently validates the attention-topology-for-failure-detection
direction (AUROC 0.71 on HotpotQA/Mistral-7B), confirming the research direction is real.
What TOHA and the broader attention-probe literature (KL-divergence probes, semantic entropy
probes) do not establish: (1) **the regime-conditional claim and its sharp limit** — topology's
utility is gated on accuracy being near 50%, decisive at 50–55% on synthetic deep-hop tasks
(Exp 13) and absent at 77–85% (Exp 7, 12); but near-50% accuracy is *necessary, not
sufficient* — the effect does not transfer to naturalistic QA (Exp 14, chance on
Mistral-7B/HotpotQA). (2) **the mechanistic decomposition by scale** — for small
models (0.5B) in the miscalibration regime, topology proxies attention entropy (r={_zp_rho_persist_entropy},
adds delta-AUC {_zp_delta_topo_beyond_ctrl} beyond controls); for larger models (1.5B),
`topo_frac_nontrivial` adds delta-AUC {zp1_delta_ctrl} beyond controls (p={zp1_p_ctrl}),
a genuinely structural signal orthogonal to entropy. The practical implication is now
appropriately narrow: this hand-specified H1 featurization is a useful failure signal only on
...
4. **Failure prediction (Spike's payoff / proposal Exp 3').** ✅ CONDITIONALLY DONE —
   ...
   NULL on distractor-augmented items. Exp 13 (Result P): **GREEN** on deep-hop items
   (accuracy {zp_acc}; topology AUC {zp_topo_all} vs confidence {zp_conf_all}), replicated
   across 3 seeds and a second model (1.5B). The finding is **regime-conditional**: topology
