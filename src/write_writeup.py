...
{zp_conf_all} (Exp 13, P, GREEN). Two contributions are novel relative to concurrent
work (TOHA, Bazarova et al. ACL 2026, which independently validates topology-for-failure
detection at AUROC 0.71 on HotpotQA/Mistral-7B): **(1) the regime-conditional claim**
— topology is useful IF AND ONLY IF the model is near its capability ceiling; and
**(2) the mechanistic decomposition by scale** — for small models topology proxies
attention entropy; for larger models it adds uniquely via `topo_frac_nontrivial`, a
genuinely structural signal. The durable contribution is the **adversarial,
...
copy relation (F/E2). As a failure predictor it is **regime-conditional**: redundant with
confidence on easy tasks (H, O — accuracy 77–85%), but adding distinctly when the model is
near its capability ceiling (P — accuracy {zp_acc}, topology AUC {zp_topo_all} vs confidence
{zp_conf_all}, GREEN in all 3 seeds and both models tested). As an **attributor**, topology
loses to attention magnitude on both induction (K) and the fairer IOI circuit (L).
...
probes) do not establish: (1) **the regime-conditional claim** — that topology's utility is
gated on accuracy being near 50%; topology adds nothing at 77–85% accuracy (Exp 7, 12) but
is decisive at 50–55% (Exp 13). (2) **the mechanistic decomposition by scale** — for small
...
a genuinely structural signal orthogonal to entropy. The practical implication: deploy
topology monitoring not as a general confidence alternative, but as a targeted signal for
tasks near a model's capability boundary.
