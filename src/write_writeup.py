...
def build(s: dict, e: dict, x: dict, r: dict, g: dict, p: dict,
          q: dict, qs: dict, qm: dict, h: dict, z: dict) -> str:
    # --- spike numbers ---
...
adds nothing beyond the model's own confidence (Exp 7, PARTIAL). Going beyond graph *shape*,
a **cellular sheaf** over the residual stream (capturing *what flows*) adds beyond H1 at
discriminating reasoning but again not at failure prediction (Exp 8, Frame 1 GREEN / Frame 2
PARTIAL). Net: topology is a real, **non-redundant diagnostic** correlate of
relational-circuit structure, and — with a strong enough instrument — a **budget-sensitive
causal** one for induction; it is not a usable pruning criterion, nor a failure predictor
distinct from model confidence.
...
def main(spike="results/real_stats.json", exp1="results/exp1_stats.json",
    ...
    open(out, "w").write(build(s, e, x, r, g, p, q, qs, qm, h, z))
    print(f"wrote {out}")
