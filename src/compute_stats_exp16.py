...
def write_findings(stats, path="FINDINGS_exp16.md"):
    """Append (idempotently) a machine-generated results section below the
    ...
    def block(cond):
        ...
        if c["verdict"] == "UNDERPOWERED":
            return f"### {cond}\n\n**Verdict: UNDERPOWERED** (base rate {c['base_rate_correct']:.3f})\n"
        a = c["auc"]
        b = c["bootstrap"]
        hs, pl = b["head_selected"], b["pooled_mtd"]
        ac, ae = (b["head_selected_beyond_confidence"],
                  b["head_selected_beyond_controls"])
        return (
            f"### {cond}\n\n"
            f"**Verdict: {c['verdict']}**  (n = {c['n_items']}, acc = {c['acc']:.3f})\n\n"
            f"| featurization | AUC | bootstrap CI95 |\n|---|---|---|\n"
            f"| head-selected MTop-Div (fold-internal top {stats['top_heads']}) | "
            f"{hs['auc']:.3f} | [{hs['ci95'][0]:.3f}, {hs['ci95'][1]:.3f}] |\n"
            f"| pooled MTop-Div | {pl['auc']:.3f} | [{pl['ci95'][0]:.3f}, {pl['ci95'][1]:.3f}] |\n"
            f"| confidence only | {a['confidence_only']['mean_auc']:.3f} | |\n"
            f"| response-entropy controls | {a['resp_entropy_controls']['mean_auc']:.3f} | |\n\n"
            f"- ΔAUC beyond confidence: {ac['delta_auc']:.3f} "
            f"(bootstrap one-sided p = {ac['p_one_sided']:.4f})\n"
            f"- ΔAUC beyond controls: {ae['delta_auc']:.3f} "
            f"(p = {ae['p_one_sided']:.4f})\n"
            f"- beats chance (CI rule): {c['head_selected_beats_chance']}; "
            f"reaches TOHA band (≥ {stats['toha_reference_band_low']}): "
            f"{c['reaches_toha_band']}\n"
        )

    section = (
        f"{RESULTS_MARKER} (generated by compute_stats_exp16 from "
        f"results/exp16_stats.json; do not edit)\n\n"
        + block("distractor") + "\n" + block("gold")
        + "\nPer the pre-registered interpretation map: RED in both conditions "
        "means the boundary is NOT this feature construction; the paper's §6 "
        "conjecture is rewritten accordingly.\n"
    )
