...
machine-checked statistics JSONs; no value is hand-typed. Code, stats JSONs, and
per-experiment verdict files: https://github.com/trunci/topo.*
...
topology is *regime-conditional*, and the regime is narrow: clean, template-generated
reasoning at a model's capability ceiling — it does not include naturalistic QA.
Within that regime topology is near-perfect where confidence fails (accuracy
{zp_acc}: topology AUC {zp_topo} vs confidence {zp_conf}; at five hops {zp_h5_topo}
vs {zp_h5_conf}; replicated across three seeds and a second model); on easy tasks
(accuracy {h_acc2}–{zo_acc_dist2}) it is fully redundant with confidence. (3) The
mechanism is scale-dependent — and deflates the headline number: at 0.5B parameters
(the scale of the {zp_h5_topo}-vs-{zp_h5_conf} result) topology proxies attention
entropy (r = {mech_rho}, adding ΔAUC = {mech_delta} beyond first-order controls),
while at 1.5B it adds ΔAUC = {zp1_dctrl} beyond those controls (item-level
bootstrap 95% CI [{zb_15_lo}, {zb_15_hi}]). **Everything else falls.** Topology loses to
...
({zt_d_auc} distractor / {zt_g_auc} gold). We offer the audit methodology — control
first-order statistics, verify instruments, benchmark against magnitude and
confidence — as the durable contribution.
...
HotpotQA with Mistral-7B-Instruct-v0.1 (a result added in the v3 revision) — the
setting our Experiments 14–16 target, using v0.3 of the same model (§7). TOHA
validates
...
margin, not a calibrated probability or a sampling-based measure such as semantic
entropy (Farquhar et al., 2024). On naturalistic QA this only makes our null
conservative — topology is at chance even against the weak baseline — but in the
synthetic regime where topology wins, a stronger sampled baseline could narrow the
reported margins; we have not tested one. The sheaf construction is non-learned. Gold-only
...
is a from-paper reimplementation of the MTop-Div metric — built to the paper's
definition with unit-tested MST equivalence, so that feature extraction ran inside
the same audited pipeline as Experiments 14–15 — rather than a run of TOHA's
released artifact; a null here bounds MTop-Div *under our
evaluation*, not TOHA's published result, and the artifact-level comparison is the
follow-up flagged in §6. All synthetic results use two task
