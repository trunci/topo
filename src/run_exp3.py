"""Experiment 3: per-head induction score, H1 topology, and first-order scalars.

Reuses the Experiment 2 GPT-2-small CPU loop verbatim and adds one column,
attn_entropy (src.residual.attention_entropy), so the residual-information test
has three first-order baselines (distance, off-diagonal mass, entropy) to control
for. Writes one row per (layer, head) to results/exp3.parquet.

Memory-safe: GPT-2 small, CPU, float32, eager attention, torch threads capped.
One model in memory, no parallel heavy work.
"""
...
def run(out_path="results/exp3.parquet", seq_len=SEQ_LEN, prefix_len=PREFIX_LEN,
    ...
    sums = {(l, h): {"ind": 0.0, "h1": 0.0, "dist": 0.0, "off": 0.0, "ent": 0.0}
        ...
        for s in range(input_ids.shape[0]):
            ...
            for l in range(L):
                ...
                for h in range(H):
                    ...
                    sums[(l, h)]["ent"] += attention_entropy(A)
            print(f"[exp3] sequence {s + 1}/{input_ids.shape[0]} done (n={n})", flush=True)
    ...
    print(f"[exp3] wrote {len(df)} head rows to {out_path}", flush=True)
    return df
