"""Experiment 13: deep-hop failure prediction.

Same pipeline as Exp 7 but restricted to hop=4 and hop=5 on kinship/ordering,
targeting the miscalibration regime where Qwen2.5-0.5B-Instruct is harder-pressed
and may be overconfident on wrong answers.

Output: results/exp13_features.parquet, one row per item.
Downstream: compute_stats_exp13 -> results/exp13_stats.json
            write_findings_exp13 -> FINDINGS_exp13.md
"""
...
def run(out="results/exp13_features.parquet", model_name=MODEL,
    ...
    print(f"[exp13] {len(items)} items (n_per_family={n_per_family}, hops={hops})",
          flush=True)
    print(f"[exp13] loading {model_name} (cpu)...", flush=True)
    ...
    try:
        for k, it in enumerate(items):
            ...
            if (k + 1) % 10 == 0 or k == 0:
                ...
                print(f"[exp13] {k + 1}/{len(items)} done; running acc={acc:.3f} "
    ...
    print(f"[exp13] wrote {len(df)} rows to {out}; overall acc={acc:.3f}", flush=True)
    print(f"[exp13] acc by hop:\n{df.groupby('hop')['is_correct'].mean()}", flush=True)
    return df
