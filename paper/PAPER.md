...
0.525: topology AUC 0.930 vs confidence 0.681; at five hops 1.000
vs 0.617; replicated across three seeds and a second model); on easy tasks
(accuracy 0.77–0.85) it is fully redundant with confidence. (3) The
mechanism is scale-dependent — and deflates the headline number: at 0.5B parameters
(the scale of the 1.000-vs-0.617 result) topology proxies attention
entropy (r = 0.925, adding ΔAUC = 0.000 beyond first-order controls),
while at 1.5B it adds ΔAUC = 0.216 beyond those controls (item-level
bootstrap 95% CI [0.147, 0.285]). **Everything else falls.** Topology loses to
...
(0.560 distractor / 0.504 gold). We offer the audit methodology — control
