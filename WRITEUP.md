...
magnitude as a circuit attributor, and **not** addable beyond confidence even when
contradictory distractors are introduced (Exp 12, O). The durable contribution is the
**adversarial, baseline-benchmarked methodology** that established all of this.
...
sentence asserting a wrong answer in the same relational role as the question (e.g. "Jack is
Will's grandfather" when gold=Zane, or "Frank is taller than Mary" when gold=Mary). The
prediction: distracted items create overconfident-wrong responses, breaking confidence
calibration and allowing topology to add. Qwen2.5-0.5B-Instruct, n=480 (240 clean, 240
distracted), same analysis as Exp 7.
...
The direct-competitor distractor **increased** accuracy by 5.4pp overall (biggest effect on
1-hop: +15pp). Rather than creating overconfident-wrong responses, the contradictory sentence
...
|---|---|---|---|---|---|
| clean (predicted: PARTIAL) | 0.768 | 0.705 | −0.003 | 0.594 | PARTIAL ✓ |
| distracted (predicted: GREEN) | 0.762 | 0.628 | −0.007 | 0.406 | PARTIAL ✗ |
...
has no gap to fill. The "poorly-calibrated confidence" hypothesis remains open for genuinely
harder tasks (e.g. multi-step factual questions or longer chains where the model is
systematically overconfident) but is not supported by synthetic kinship/ordering with
...
- Result O (Exp 12) used direct-competitor distractors (contradictory sentence in same
  relational role as the question) to try to create poorly-calibrated confidence. The
  distractor instead improved accuracy (+5.4pp overall), indicating the model robustly
  resolves contradictions. Single model (Qwen2.5-0.5B-Instruct), 480 items, single seed.
...
exp9b_length, exp11, and exp12). Regenerate this document with `uv run python -m src.write_writeup`.
